...
from trainer.metrics.base_metrics_class import Recorder
...
class Trainer(object):
    ...
    def DP_speed_up(self):
        self.model.device = device
        self.model.to(device)
        self.model = DataParallel(self.model,device_ids=[0,1,2,3], output_device=0)
    ...
    def train_epoch(
        ...
        test_step = 150 #len(train_data_loader) // times_per_epoch; 150
    ...
    def test_one_dataset(self, data_loader):
        ...
        for i, data_dict in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            for key in data_dict.keys():
                if data_dict[key] != None and key != 'name':
                    data_dict[key] = data_dict[key].cuda(device=device)
            # model forward without considering gradient computation
            predictions = self.inference(data_dict)
            label_lists = label_lists + list(data_dict['label'].cpu().detach().numpy())
            prediction_lists = prediction_lists + list(predictions['prob'].cpu().detach().numpy())
    ...
    def save_best(self, epoch, iteration, step, losses_one_dataset_recorder, key, metric_one_dataset):
        ...
        if losses_one_dataset_recorder is not None:
            ...
            for k, v in losses_one_dataset_recorder.items():
                ...
                loss_str = loss_str + f"testing-loss, {k}: {v_avg}    "
            self.logger.info(loss_str)
        ...
        for k, v in metric_one_dataset.items():
            if k == 'pred' or k == 'label' or k == 'dataset_dict':
                continue
            metric_str = metric_str + f"testing-metric, {k}: {v}    "
            # tensorboard-2. metric
            writer = self.get_writer('test', key, k)
    ...
    def test_epoch(self, epoch, iteration, test_data_loaders, step):
        ...
        if len(keys) > 0 and self.config.get('save_avg', False):
            ...
            for key in avg_metric:
                if key != 'dataset_dict':
                    avg_metric[key] = avg_metric[key]/len(keys)
