    ...
    def __init__(
        ...
        self.resblocks = nn.ModuleList([
            ResidualAttentionBlock(
                width, heads, attn_mask, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer,
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(torch.min(attn_ori))
        # print(torch.max(attn_ori))
        
        attn_ori = (attn_ori).softmax(dim=-1) # .softmax(dim=-1)
        
        # print("NAN", torch.isnan(attn_ori).any())
        # print("INF", torch.isinf(attn_ori).any())
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list):
        ...
        if self.attn == None:
            ...
            for i in range(1, 20):  # surgery 7, maskclip 2
